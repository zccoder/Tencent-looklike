...
def Feature_len(train,predict,feat):
	train[feat+'_len']=train[feat].apply(lambda x:len(x.split()) if x!='-1' and x!='0' else 0)
	predict[feat+'_len']=predict[feat].apply(lambda x:len(x.split()) if x!='-1' and x!='0' else 0)
...
all_train.to_csv("vector_len_feature_train.csv",index=None)
all_test.to_csv("vector_len_feature_test.csv",index=None)
