...
usetruecols=usecols[10:14]
usetruecols.append(usecols[-1])

feat_List=usetruecols[:-1]

train=pd.read_csv(input_path+"train_two_single_3_3.csv",usecols=usetruecols)
test=pd.read_csv(input_path+"test_two_single_3_3.csv",usecols=usetruecols)
...
all_train.to_csv(save_path+"two_single_statis_feature_train_3_3.csv",index=None)
all_test.to_csv(save_path+"two_single_statis_feature_test_3_3.csv",index=None)
t2=datetime.now()
print(t2-t1)
