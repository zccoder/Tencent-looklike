...
#16
user_feat_list=['marriageStatus','interest1','interest2','interest3','interest4',
'interest5','kw1','kw2','kw3','topic1','topic2','topic3','appIdInstall','appIdAction','ct','os']
all_train=None
all_test=None
for afeat in ad_feat_list:
    if afeat=='adCategoryId':#已过一半，保存
        all_train.to_csv("adcacr_usermultivalue_cross_statis_feature_train.csv",index=None)#分两部分保存
        all_test.to_csv("adcacr_usermultivalue_cross_statis_feature_test.csv",index=None)
        all_train=None
        all_test=None
        temp=datetime.now()
        print(temp-t1)
    for ufeat in user_feat_list:
        feat=afeat+'_'+ufeat+'_multi'
        train[feat]=train[[afeat,ufeat]].apply(lambda x:hebing(x[afeat],x[ufeat]),axis=1)
        predict[feat]=predict[[afeat,ufeat]].apply(lambda x:hebing(x[afeat],x[ufeat]),axis=1)
...
all_train.to_csv("adpro2_usermultivalue_cross_statis_feature_train.csv",index=None)
all_test.to_csv("adpro2_usermultivalue_cross_statis_feature_test.csv",index=None)
