...
single_hand_onehot1_index=['aid_age','aid_carrier','aid_consumptionAbility','aid_education','aid_gender','aid_house','aid_os','aid_ct']
single_hand_onehot1_max=[4826,3327,2495,6651,2458,1663,4159,50810]

single_hand_onehot2_index=['aid_marriageStatus','aid_advertiserId','aid_campaignId','aid_creativeId','aid_adCategoryId','aid_productId','aid_productType','aid_creativeSize']
single_hand_onehot2_max=[14614,831,831,831,831,831,831,831]
...
aid_marriageStatus 14614
aid_advertiserId 831
aid_campaignId 831
aid_creativeId 831
aid_adCategoryId 831
aid_productId 831
aid_productType 831
aid_creativeSize 831
...
def build_model():
    ...
    for c in range(len(single_hand_onehot1_index)):
        inputs_c = Input(shape=(1,), dtype='int32',name = 'input_%s'%single_hand_onehot1_index[c])
        num_c = single_hand_onehot1_max[c]+1
        inputs.append(inputs_c)
        print (num_c,c)
        embed_c = Embedding(
                        num_c,
                        emb_n,
                        input_length=1,
                        name = 'embed_%s'%single_hand_onehot1_index[c]
                        #W_regularizer=l2_reg(l2_fm)
                        )(inputs_c)
    ...
    for c in range(len(single_hand_onehot2_index)):
        inputs_c = Input(shape=(1,), dtype='int32',name = 'input_%s'%single_hand_onehot2_index[c])
        num_c = single_hand_onehot2_max[c]+1
        inputs.append(inputs_c)
        print (num_c,c)
        embed_c = Embedding(
                        num_c,
                        emb_n,
                        input_length=1,
                        name = 'embed_%s'%single_hand_onehot2_index[c]
                        #W_regularizer=l2_reg(l2_fm)
                        )(inputs_c)
    ...
    for f in range(len(model_col_name)):
        ...
        flatten_f = Reshape((emb_n2,))(embed_f)
  ...
  #       squared_features_emb.append(multiply([layer,layer]))
   ...
   # y_second_order = Dropout(0.8)(y_second_order)
    ...
    concat_input = concatenate([fm_layers,y_deep],axis=1)
   # concat_input = concatenate([fm_layers,y_deep],axis=1)
...
#------------------------------------------------------------------------------
t1=time.time()
single_onehot=pd.read_csv("./SDD_data/final_sdd_single_onehot_embedding_feature2.csv")
...
del test_set,evals_x,train_x;gc.collect()
print('input data over!',time.time()-t1)
#-**************************************************************************************

#-**************************************************************************************
single_onehot=pd.read_csv("./SDD_data/final_sdd_handcraft_onehot_embedding_feature3.csv")
train_set=single_onehot[single_onehot['label']!=-1].values[:,3:] #train data
...
for i in range(len(single_hand_onehot1_index)-1):
   X_train.append(train_x[:,i:i+1])
   X_valid.append(evals_x[:,i:i+1])
   X_test.append(test_set[:,i:i+1])
X_train.append(train_x[:,(len(single_hand_onehot1_index)-1):])
X_valid.append(evals_x[:,(len(single_hand_onehot1_index)-1):])
X_test.append(test_set[:,(len(single_hand_onehot1_index)-1):])##
del test_set,evals_x,train_x;gc.collect()
print('input data over!',time.time()-t1)
#-**************************************************************************************
...
#-**************************************************************************************
single_onehot=pd.read_csv("./SDD_data/final_sdd_handcraft_onehot_embedding_feature4.csv")
train_set=single_onehot[single_onehot['label']!=-1].values[:,3:] #train data
...
for i in range(len(model_col_name)):
   temp_file=pd.read_csv("./SDD_data/final_sdd_embedding_feature_mix_chusai_%s.csv"%model_col_name[i])
...
y_pred_d = model.predict(X_valid,batch_size=10000)  
...
subfile['label']=pre1
subfile.to_csv("./sdd_results/63_add_handf_3_1.csv",header=True,index=False)
