...
allfeature=['aid','age','gender','education','consumptionAbility','LBS','carrier','house','advertiserId','campaignId','creativeId','adCategoryId']
...
for fea in allfeature:
	...
	train_b=pd.merge(train_b,xx_feature,on=fea,how='left')
	predict=pd.merge(predict,xx_feature,on=fea,how='left')
	...
	xx_feature["count_%s"%fea]=xx_feature['count_%s'%fea].replace(np.nan,0)
	xx_feature['%s_click_rate'%fea]=xx_feature['count_click_%s'%fea].astype('float')/xx_feature["count_%s"%fea].astype('float')
	train_a=pd.merge(train_a,xx_feature,on=fea,how='left')
...
train_ab.to_csv("train_single_value_statistics_feature.csv",index=None)
predict.to_csv("test_single_value_statistics_feature.csv",index=None)
