...
def fit_predict(xs,model_path, y_train):
    ...
    with tf.Session(graph=tf.Graph()) as sess, timer('fit_predict'):
        ...
        deep = ks.layers.Dense(384, activation='relu')(deep)
        ...
        for i in range(2):
            with timer(f'epoch {i + 1}'):
                model.fit(x=all_split_train_x, y=y_train,batch_size=8192, epochs=1, verbose=2)
        model.save_weights(model_path+"model_weigth_{0}.h5".format(index))
        model.save(model_path+"model_{0}.h5".format(index))
...
train_transfer=pd.read_csv(transfer_path+"three_multi_statis_feature_train_1_2.csv")
train_transfer1=pd.read_csv(transfer_path+"vector_statis_feature_train.csv")
...
test_transfer=pd.read_csv(transfer_path+"three_multi_statis_feature_test_1_2.csv")
test_transfer=test_transfer.fillna(test_transfer.median())
test_transfer1=pd.read_csv(transfer_path+"vector_statis_feature_test.csv")
...
res.to_csv(save_path+'cv_NN1.csv', index=False)
