...
def build_model():
  ...
  #       squared_features_emb.append(multiply([layer,layer]))
   ...
   # y_second_order = Dropout(0.8)(y_second_order)
    ...
    new_input = concatenate([fm_layers,y_deep],axis=1)
    new_input = Dense(256)(new_input)  #加入BN会变差  PReLU 优于Relu 0.5 is 7483
    new_input = Activation('relu',name='output_3')(new_input)
    new_input = Dropout(0.5)(new_input)
    new_input=  Dense(128)(new_input)
    new_input = Activation('relu',name='output_4')(new_input)
    new_input = Dropout(0.5)(new_input)	
